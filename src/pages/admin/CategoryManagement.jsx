...
const CategoryManagement = () => {
  ...
  const handleUpdateCategory = async (e) => {
    ...
    try {
      ...
      const updatedCategory = await updateCategory(editCategory.id, categoryData);
      ...
      setFormError('Đã xảy ra lỗi khi cập nhật danh mục. Vui lòng thử lại sau.');
    } finally {
  ...
  const handleDeleteClick = (category) => {
    if (category.documentCount > 0) {
      setErrorMessage(`Không thể xóa danh mục "${category.title}" vì còn chứa ${category.documentCount} tài liệu. Vui lòng xóa tài liệu trước.`);
  ...
  const handleDeleteCategory = async () => {
    ...
    try {
      ...
      setDeleteSuccess(`Đã xóa danh mục "${categoryToDelete.title}" thành công!`);
  ...
  <div className={`${isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white'}`}>
    <div className={`px-6 py-4 ${isDarkMode ? 'bg-gray-700/50 border-b border-gray-700' : 'bg-gray-50 border-b border-gray-200'}`}>
      ...
      <p className={`mt-1 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
        {loading ? 'Đang tải...' : `Hiển thị ${filteredCategories.length} / ${categories.length} danh mục trong hệ thống`}
      </p>
    </div>
    ...
    {loading ? (
      ...
      <div className="text-center py-12">
        ...
        <p className={`mt-2 text-sm font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
      ...
      <>
          ...
          <table className={`min-w-full divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
            <thead>
              <tr className={isDarkMode ? 'bg-gray-700/30' : 'bg-gray-50'}>
                <th scope="col" className={`px-3 py-3 text-left text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>
                  STT
                </th>
                <th scope="col" className={`px-3 py-3 text-left text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>
                  Danh mục
                </th>
                <th scope="col" className={`px-3 py-3 text-right text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>
            ...
            <tbody className={`divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
              {filteredCategories.map((category, index) => (
                <tr key={category.id} className={isDarkMode ? 'hover:bg-gray-700/30' : 'hover:bg-gray-50'}>
                  ...
                  <td className="px-3 py-3">
                    <div className="flex items-center">
                      <div className={`w-8 h-8 flex-shrink-0 mr-2 flex items-center justify-center rounded-md ${isDarkMode ? 'bg-gray-600' : 'bg-blue-100'}`}>
                        <div className={`h-5 w-5 ${isDarkMode ? 'text-blue-300' : 'text-blue-600'}`}>
                  ...
                  <td className="px-3 py-3 whitespace-nowrap text-right text-sm">
                    <div className="flex justify-end space-x-2">
                      ...
                      <button
                        ...
                        className={`p-1.5 rounded-md ${
                          category.documentCount > 0 ? (isDarkMode ? 'text-gray-600 cursor-not-allowed' : 'text-gray-400 cursor-not-allowed') : (isDarkMode ? 'text-red-400 hover:bg-gray-700 hover:text-red-300' : 'text-red-600 hover:bg-gray-100 hover:text-red-700')
                        }`}
                        title={category.documentCount > 0 ? "Không thể xóa danh mục có tài liệu" : "Xóa danh mục"}
        ...
        <div className="hidden md:block w-full overflow-x-auto" style={{ WebkitOverflowScrolling: 'touch' }}>
          <table className={`min-w-full divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
            <thead>
              <tr className={isDarkMode ? 'bg-gray-700/30' : 'bg-gray-50'}>
                <th scope="col" className={`sticky top-0 px-4 sm:px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-300 bg-gray-700/50' : 'text-gray-500 bg-gray-50'}`}>
                  STT
                </th>
                <th scope="col" className={`sticky top-0 px-4 sm:px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-300 bg-gray-700/50' : 'text-gray-500 bg-gray-50'}`}>
                  Biểu tượng
                ...
                <th scope="col" className={`sticky top-0 px-4 sm:px-6 py-3 text-right text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-300 bg-gray-700/50' : 'text-gray-500 bg-gray-50'}`}>
                  <span className="sr-only">Thao tác</span>
                </th>
              </tr>
            </thead>
              ...
              {filteredCategories.map((category, index) => (
                ...
                <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center justify-center">
                    <div className={`w-8 h-8 flex items-center justify-center rounded-md ${isDarkMode ? 'bg-gray-600' : 'bg-blue-100'}`}>
                      {/* Render the proper icon based on category.logo from Firestore */}
                      <div className={`h-5 w-5 ${isDarkMode ? 'text-blue-300' : 'text-blue-600'}`}>
                        {renderCategoryIcon(category.logo)}
                      </div>
                    </div>
                ...
                <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                  ...
                  {/* Show document count on mobile only */}
                  <div className={`text-xs mt-1 md:hidden ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    {category.documentCount || 0} tài liệu
                  </div>
                </td>
                ...
                <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm">
                  <div className="flex items-center justify-end space-x-2">
                    ...
                    <button
                      onClick={() => category.documentCount > 0 ? handleDeleteClick(category) : handleDeleteClick(category)}
                      className={`inline-flex items-center p-1.5 rounded-md ${
                      ...
                      {newCategory.logo 
                        ? <div className="flex items-center">
                            ...
                            <span className="mr-2 text-gray-700 dark:text-gray-300">
                              {academicLogos.find(l => l.id === newCategory.logo)?.icon}
                            </span>
                            <span className="font-medium">{academicLogos.find(l => l.id === newCategory.logo)?.name}</span>
                          </div>
                    ...
                    <div className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                      {editCategory.logo 
                        ? <div className="flex items-center">
                            <span className="mr-2">Đã chọn:</span>
                            <span className="mr-2 text-gray-700 dark:text-gray-300">
                              {academicLogos.find(l => l.id === editCategory.logo)?.icon}
                            </span>
                            <span className="font-medium">{academicLogos.find(l => l.id === editCategory.logo)?.name}</span>
                          </div>
                        : "Vui lòng chọn một biểu tượng cho danh mục"
                      }
                    </div>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            >
                ...
                {deleteSuccess ? (
                  ...
                  <>
                    ...
                    <p className={`text-center text-sm mb-5 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      Bạn có chắc chắn muốn xóa danh mục <span className="font-semibold">{categoryToDelete?.title}</span>?
                      <br />
              ...
              {/* Modal Footer */}
              <div className={`px-6 py-4 sm:flex ${deleteSuccess ? 'justify-center' : 'sm:flex-row-reverse'} ${isDarkMode ? 'bg-gray-700 border-t border-gray-600' : 'bg-gray-50 border-t border-gray-200'}`}>
                {deleteSuccess ? (
                  <button
                    type="button"
                    onClick={() => setShowDeleteModal(false)}
