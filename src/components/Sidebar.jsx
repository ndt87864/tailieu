...
const getCategoryIcon = (logoId) => {
  ...
  const logoMap = {
    'business': (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 flex-shrink-0 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const getLogoIcon = (logoId) => {
  ...
  if (!logoId) {
    return (
      <svg className="w-4 h-4 flex-shrink-0 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  const logoMap = {
    'business': (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 flex-shrink-0 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
      </svg>
    ),
    // Add all other logo mappings here, same as for categories but with w-4 h-4 class
    'economics': (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 flex-shrink-0 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
      </svg>
...
function Sidebar({ 
  ...
  const handleCategoryClick = async (idx, category) => {
    // Toggle the category - if it's already open, close it
    if (openMain === idx) {
      setOpenMain(-1); // Set to -1 to close the currently open category
      return; // Exit early - no need to fetch documents or navigate
    }
    
    // Đặt danh mục hiện tại là mở
    setOpenMain(idx);
    
    // Kiểm tra xem đã có tài liệu trong state chưa
    if (!documents[category.id]) {
      try {
        ...
        // Nếu có documents, chuyển đến document đầu tiên
        if (docsData.length > 0) {
          const categorySlug = toSlug(category.title);
          navigate(`/${categorySlug}/${docsData[0].slug}`);
      ...
      // Nếu đã có tài liệu trong state, chuyển hướng ngay đến tài liệu đầu tiên
      const existingDocs = documents[category.id];
      if (existingDocs && existingDocs.length > 0) {
        const categorySlug = toSlug(category.title);
        navigate(`/${categorySlug}/${existingDocs[0].slug}`);
        setSelectedDocument(existingDocs[0]);
      }
    }
  };
  ...
  const sidebarClasses = `
    h-screen overflow-y-auto border-r ${isDarkMode ? 'border-gray-700' : 'border-green-800'} transition-all duration-300 scrollbar-hide
    ...
    <>
      ...
      <aside className={sidebarClasses}>
        ...
        <div className={`sticky top-0 theme-sidebar z-10`}>
          <div className={`p-4 flex items-center justify-between border-b ${isDarkMode ? 'border-gray-700' : 'border-green-600'}`}>
            <Link to="/" className="flex items-center overflow-hidden">
              ...
              {showFullContent && <span className="text-lg font-medium truncate">Tài liệu HOU</span>}
            </Link>
          ...
          {isAdmin && (
            <div className={`border-b ${isDarkMode ? 'border-gray-700' : 'border-green-600'}`}>
              <Link 
                to="/admin/users"
              ...
              >
                ...
                {showFullContent && <span className="text-white font-medium">Quản lý tài khoản</span>}
              </Link>
              ...
              >
                ...
                {showFullContent && <span className="text-white font-medium">Quản lý danh mục</span>}
              </Link>
              ...
              >
                ...
                {showFullContent && <span className="text-white font-medium">Quản lý tài liệu</span>}
              </Link>
              ...
              >
                ...
                {showFullContent && <span className="text-white font-medium">Quản lý bộ câu hỏi</span>}
              </Link>
            </div>
          )}
        </div>
        ...
        <nav className="mt-2 px-2 overflow-y-auto">
          <ul className="space-y-1">
            {!hideDocumentTree && sidebarData && sidebarData.map((category, idx) => (
              <li key={category.id || idx} className="rounded-md overflow-hidden">
                <button
                  className={`w-full flex items-center ${isCollapsed && windowWidth >= 770 ? 'justify-center' : 'justify-between'} px-3 py-2.5 text-sm font-medium text-left transition-all duration-200 rounded-md ${
                    openMain === idx 
                      ? isDarkMode ? "bg-slate-700 text-white" : "theme-category-selected text-white" 
                      : `text-white hover:${isDarkMode ? 'bg-slate-700' : 'theme-button-hover'} hover:shadow-md`
                  }`}
                  style={openMain === idx && isDarkMode ? {backgroundColor: '#334155'} : {}}
                ...
                >
                  <div className="flex items-center flex-1 min-w-0">
                    ...
                    {showFullContent && <span className="truncate ml-2 mr-2">{category.title}</span>}
                  </div>
                ...
                {openMain === idx && documents && documents[category.id] && (
                  <ul className={`mt-1 ${showFullContent ? 'pl-4' : ''} space-y-0.5 py-1 ${showFullContent ? (isDarkMode ? 'bg-slate-700/50' : 'theme-category-container') + ' rounded-md mx-1' : ''}`} 
                    style={showFullContent && isDarkMode ? {backgroundColor: 'rgba(51, 65, 85, 0.5)'} : {}}
                  >
                    {documents[category.id].map((doc) => (
                      <li key={doc.id}>
                        <button
                          className={`group flex items-center w-full ${showFullContent ? 'px-3' : 'px-1'} py-2 text-sm rounded-md transition-all duration-200 ${
                            isCollapsed && windowWidth >= 770 && windowWidth < 1280 ? 'justify-center' : ''
                          } ${
                            selectedDocument && selectedDocument.id === doc.id 
                              ? showFullContent ? (isDarkMode ? "bg-slate-600/50" : "theme-document-selected") + " text-white font-medium shadow-sm" : "text-white font-medium"
                              : `text-white hover:${isDarkMode ? 'bg-slate-700' : 'theme-button-hover'} hover:text-white hover:shadow-sm`
                          }`}
                          style={selectedDocument && selectedDocument.id === doc.id && showFullContent && isDarkMode ? {backgroundColor: 'rgba(71, 85, 105, 0.5)'} : {}}
                          onClick={() => handleDocumentClick(category, doc)}
                        >
                          {getLogoIcon(doc.categoryLogo)}
                          {showFullContent && <span className="truncate flex-1 min-w-0 ml-2">{doc.title}</span>}
                        </button>
                      </li>
                    ))}
                  </ul>
